...
@defcomp contrails_rf begin
    ...
    NOx_emissions     = Parameter(index=[time]) # Nitrogen oxides emissions.
    ...
    function run_timestep(p, v, d, t)
        ...
        if p.ref_is_NO2
            v.forcing_contrails[t] = p.NOx_emissions[t] * p.frac[t] * (p.F_ref/p.E_ref) * (p.mol_weight_NO₂ / p.mol_weight_N)
        else
            v.forcing_contrails[t] = p.NOx_emissions[t] * p.frac[t] * (p.F_ref/p.E_ref)
        end
    end
end
