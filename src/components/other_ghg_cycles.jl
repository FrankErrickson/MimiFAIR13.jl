...
@defcomp other_ghg_cycles begin
    ...
    other_ghg_τ                = Parameter(index=[other_ghg])                       # Atmospheric (e-folding) lifetime for each gas (years).
    ...
    emissions_other_ghg        = Parameter(index=[time, other_ghg])                 # Emissions for other well-mixed greenhouse gases (kt yr⁻¹).
    ...
    function run_timestep(p, v, d, t)
        ...
        for g in d.other_ghg
            ...
            if is_first(t)
               ...
               v.conc_other_ghg[t,g] = v.conc_other_ghg[t-1,g] - v.conc_other_ghg[t-1,g] * (1.0 - exp(-1/p.other_ghg_τ[g])) + 0.5 * (p.emissions_other_ghg[t-1,g] + p.emissions_other_ghg[t,g]) * (1.0/p.emiss2conc_other_ghg[g])
            end
        end
