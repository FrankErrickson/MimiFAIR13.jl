...
@defcomp co2_cycle begin
    ...
    CO₂_0    = Parameter()              # Pre-industrial carbon dioxide concentrations (ppm)
    ...
    CO₂_τ    = Parameter(index=[4])     # Decay time constant for each carbon pool in 'a'
    ...
    function run_timestep(p, v, d, t)
        ...
        if is_first(t)
            ...
            function f!(F, x)
                F[1] = -v.iIRFT100[t] + sum(x[1] .* p.a .* p.CO₂_τ .* (1 .- exp.(-100 ./ x[1] ./ p.CO₂_τ)))
            end
