...
function constructfair(;rcp_scenario::String="RCP85", start_year::Int64=1765, end_year::Int64=2500, F2x::Float64=3.71, TCR::Float64=1.6, ECS::Float64=2.75, d::Array{Float64,1}=[239.0, 4.1])
    ...
    # ---- Methane Cycle ---- #
    set_param!(m, :ch4_cycle, :fossil_emiss, rcp_emissions.CH4)
    set_param!(m, :ch4_cycle, :natural_emiss, rcp_emissions.NaturalCH4)
    set_param!(m, :ch4_cycle, :CH₄_0, gas_data[findfirst(gas_data[!,:gas] .== "CH4"), :pi_conc])
    set_param!(m, :ch4_cycle, :CH₄_τ, 9.3)
    set_param!(m, :ch4_cycle, :fossil_frac, gas_fractions.ch4_fossil)
    set_param!(m, :ch4_cycle, :oxidation_frac, 0.61)
    set_param!(m, :ch4_cycle, :mol_weight_CH₄, gas_data[findfirst(gas_data[!,:gas] .== "CH4"), :mol_weight])
    set_param!(m, :ch4_cycle, :mol_weight_CO₂, gas_data[findfirst(gas_data[!,:gas] .== "CO2"), :mol_weight])
    set_param!(m, :ch4_cycle, :emiss2conc_ch4, conversions[findfirst(conversions[!,:gases] .== "CH4"), :emiss2conc])

    # ---- Nitrous Oxide Cycle ---- #
    set_param!(m, :n2o_cycle, :fossil_emiss, rcp_emissions.N2O)
    set_param!(m, :n2o_cycle, :natural_emiss, rcp_emissions.NaturalN2O)
    set_param!(m, :n2o_cycle, :N₂O_0, gas_data[findfirst(gas_data[!,:gas] .== "N2O"), :pi_conc])
    set_param!(m, :n2o_cycle, :N₂O_τ, 121.0)
    set_param!(m, :n2o_cycle, :emiss2conc_n2o, conversions[findfirst(conversions[!,:gases] .== "N2O"), :emiss2conc])


    # ---- Carbon Cycle ---- #
    set_param!(m, :co2_cycle, :CO2_0, gas_data[findfirst(gas_data[!,:gas] .== "CO2"), :pi_conc])
    ...
    set_param!(m, :co2_cycle, :CO₂_τ, [10.0^6, 394.4, 36.54, 4.304])
    set_param!(m, :co2_cycle, :E, (rcp_emissions[!,:FossilCO2] .+ rcp_emissions[!,:OtherCO2]))
    set_param!(m, :co2_cycle, :gtc2ppm, conversions[findfirst(conversions[!,:gases] .== "CO2"), :emiss2conc])

    # ---- Other Well-Mixed Greenhouse Gas Cycles ---- #
    set_param!(m, :other_ghg_cycles, :other_ghg_τ, gas_data[findall((in)(other_ghg_names), gas_data[!,:gas]), :lifetimes])
    ...
    set_param!(m, :ch4_rf, :N₂O_0, gas_data[findfirst(gas_data[!,:gas] .== "N2O"), :pi_conc])
    set_param!(m, :ch4_rf, :H₂O_share, 0.12)
    set_param!(m, :ch4_rf, :scale_CH₄, 1.0)
    set_param!(m, :ch4_rf, :CH₄_0, gas_data[findfirst(gas_data[!,:gas] .== "CH4"), :pi_conc])
    ...
    set_param!(m, :n2o_rf, :N₂O_0, gas_data[findfirst(gas_data[!,:gas] .== "N2O"), :pi_conc])
    set_param!(m, :n2o_rf, :CO₂_0, gas_data[findfirst(gas_data[!,:gas] .== "CO2"), :pi_conc])
    set_param!(m, :n2o_rf, :CH₄_0, gas_data[findfirst(gas_data[!,:gas] .== "CH4"), :pi_conc])

    # ---- Carbon Dioxide Radiative Forcing ---- #
    set_param!(m, :co2_rf, :a₁, -2.4e-7)
    set_param!(m, :co2_rf, :b₁, 7.2e-4)
    set_param!(m, :co2_rf, :c₁, -2.1e-4)
    set_param!(m, :co2_rf, :CO₂_0, gas_data[findfirst(gas_data[!,:gas] .== "CO2"), :pi_conc])
    set_param!(m, :co2_rf, :N₂O_0, gas_data[findfirst(gas_data[!,:gas] .== "N2O"), :pi_conc])
    set_param!(m, :co2_rf, :scale_CO₂, scale_co2_forcing)
    ...
    set_param!(m, :trop_o3_rf, :mol_weight_N, gas_data[findfirst(gas_data[!,:gas] .== "N"), :mol_weight])
    set_param!(m, :trop_o3_rf, :mol_weight_NO, gas_data[findfirst(gas_data[!,:gas] .== "NO"), :mol_weight])
    set_param!(m, :trop_o3_rf, :CH₄_0, gas_data[findfirst(gas_data[!,:gas] .== "CH4"), :pi_conc])
    ...
    set_param!(m, :aerosol_indirect_rf, :rcp_1850_index, findfirst(x -> x == 1850, collect(1765:end_year)))
    ...
    set_param!(m, :contrails_rf, :mol_weight_NO₂, gas_data[findfirst(gas_data[!,:gas] .== "NO2"), :mol_weight])
    set_param!(m, :contrails_rf, :mol_weight_N, gas_data[findfirst(gas_data[!,:gas] .== "N"), :mol_weight])

    # ---- Total Radiative Forcing ---- #
    set_param!(m, :total_rf, :F_volcanic, volcano_forcing)
    set_param!(m, :total_rf, :F_solar, solar_forcing)
    set_param!(m, :total_rf, :F_exogenous, zeros(nsteps))
