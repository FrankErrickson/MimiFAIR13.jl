...
@defcomp trop_o3_rf begin
    ...
    model_years           = Parameter(index=[time]) # Years for model time-horizon (ensures correct equations are used since some equations are year-depednent).
...
function run_timestep(s::trop_o3_rf, t::Int)
    v = s.Variables
    p = s.Parameters
